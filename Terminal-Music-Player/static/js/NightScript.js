...
let night; // Holds the loaded music data
function setupMediaSessionControls() {
  if ('mediaSession' in navigator) {
    navigator.mediaSession.metadata = new MediaMetadata({
      title: night[track_index].name,
      artist: night[track_index].artist,
      // Include other metadata information here
      artwork: [
        { src: night[track_index].img, sizes: '96x96', type: 'image/jpeg' },
      ],
    });
...
function loadTrack(track_index) {
  ...
  let trackName = night[track_index].name;
  if (trackName.length > 30) {
    trackName = trackName.substring(0, 25) + "...";
  }

  track_name.textContent = trackName;
  track_artist.textContent = night[track_index].artist;
  now_playing.textContent = "Playing music " + (track_index + 1) + " of " + night.length;

  let addedBy = night[track_index]['added_by'];
...
function nextTrack() {
  if (track_index < night.length - 1 && isRandom === false) {
    track_index += 1;
  } else if (track_index < night.length - 1 && isRandom === true) {
    let random_index = Number.parseInt(Math.random() * night.length);
...
downloadBtn.addEventListener('click', function() {
  let currentTrack = night[track_index];
