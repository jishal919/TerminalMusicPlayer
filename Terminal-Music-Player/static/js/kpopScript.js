...
let kpop; // Holds the loaded music data
function setupMediaSessionControls() {
  if ('mediaSession' in navigator) {
    navigator.mediaSession.metadata = new MediaMetadata({
      title: kpop[track_index].name,
      artist: kpop[track_index].artist,
      // Include other metadata information here
      artwork: [
        { src: kpop[track_index].img, sizes: '96x96', type: 'image/jpeg' },
      ],
    });
...
function loadTrack(track_index) {
  ...
  let trackName = kpop[track_index].name;
  if (trackName.length > 30) {
    trackName = trackName.substring(0, 25) + "...";
  }

  track_name.textContent = trackName;
  track_artist.textContent = kpop[track_index].artist;
  now_playing.textContent = "Playing music " + (track_index + 1) + " of " + kpop.length;

  let addedBy = kpop[track_index]['added_by'];
...
function nextTrack() {
  if (track_index < kpop.length - 1 && isRandom === false) {
    track_index += 1;
  } else if (track_index < kpop.length - 1 && isRandom === true) {
    let random_index = Number.parseInt(Math.random() * kpop.length);
...
downloadBtn.addEventListener('click', function() {
  let currentTrack = kpop[track_index];
