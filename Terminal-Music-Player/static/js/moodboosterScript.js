...
let moodbooster; // Holds the loaded music data
function setupMediaSessionControls() {
  if ('mediaSession' in navigator) {
    navigator.mediaSession.metadata = new MediaMetadata({
      title: moodbooster[track_index].name,
      artist: moodbooster[track_index].artist,
      // Include other metadata information here
      artwork: [
        { src: moodbooster[track_index].img, sizes: '96x96', type: 'image/jpeg' },
      ],
    });
...
fetch(apiUrl, {
  ...
  .then(data => {
    ...
    moodbooster = jsonData.moodbooster;
...
function loadTrack(track_index) {
  ...
  let imageUrl = moodbooster[track_index].img + '?' + randomQuery;
  track_art.style.backgroundImage = "url(" + imageUrl + ")";

  // Load the audio element with the music source
  curr_track.src = moodbooster[track_index].music;
  ...
  let trackName = moodbooster[track_index].name;
  if (trackName.length > 30) {
    trackName = trackName.substring(0, 25) + "...";
  }

  track_name.textContent = trackName;
  track_artist.textContent = moodbooster[track_index].artist;
  now_playing.textContent = "Playing music " + (track_index + 1) + " of " + moodbooster.length;

  let addedBy = moodbooster[track_index]['added_by'];
...
function nextTrack() {
  if (track_index < moodbooster.length - 1 && isRandom === false) {
    track_index += 1;
  } else if (track_index < moodbooster.length - 1 && isRandom === true) {
    let random_index = Number.parseInt(Math.random() * moodbooster.length);
...
function prevTrack() {
  if (track_index > 0) {
    ...
    track_index = moodbooster.length - 1;
  }
  loadTrack(track_index);
  playTrack();
  setupMediaSessionControls()
...
downloadBtn.addEventListener('click', function() {
  let currentTrack = moodbooster[track_index];
