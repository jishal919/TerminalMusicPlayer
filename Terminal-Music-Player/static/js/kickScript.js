...
let kick; // Holds the loaded music data
function setupMediaSessionControls() {
  if ('mediaSession' in navigator) {
    navigator.mediaSession.metadata = new MediaMetadata({
      title: kick[track_index].name,
      artist: kick[track_index].artist,
      // Include other metadata information here
      artwork: [
        { src: kick[track_index].img, sizes: '96x96', type: 'image/jpeg' },
      ],
    });
...
function loadTrack(track_index) {
  ...
  let trackName = kick[track_index].name;
  if (trackName.length > 30) {
    trackName = trackName.substring(0, 25) + "...";
  }

  track_name.textContent = trackName;
  track_artist.textContent = kick[track_index].artist;
  now_playing.textContent = "Playing music " + (track_index + 1) + " of " + kick.length;

  let addedBy = kick[track_index]['added_by'];
...
function nextTrack() {
  if (track_index < kick.length - 1 && isRandom === false) {
    track_index += 1;
  } else if (track_index < kick.length - 1 && isRandom === true) {
    let random_index = Number.parseInt(Math.random() * kick.length);
...
downloadBtn.addEventListener('click', function() {
  let currentTrack = kick[track_index];
